...
class PlayState extends MusicBeatState
{
					...
					if (StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'thorns')
					{
						...
						new FlxTimer().start(0, function(swagTimer:FlxTimer)
						{
							...
							else
							{
								...
								FlxG.sound.play(Paths.sound('Senpai_Dies'), 0, false, null, true, function()
								{
									remove(senpaiEvil);
									remove(red);
	...
	if (isStoryMode)
	{
		if (FlxG.save.data.scoreScreen)
			{
				if (SONG.song.toLowerCase() == 'your choice' || SONG.song.toLowerCase() == 'training')
				{
	...
	function endSong():Void
	{
		...
		else
		{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					vocals.stop();
						if (FlxG.save.data.scoreScreen)
							...
							trace('ending week with end dialogue lmao');
								if (SONG.song.toLowerCase() == 'your choice' || SONG.song.toLowerCase() == 'training')
								{
									FlxG.sound.playMusic(Paths.music('freakyMenu'));
									FlxG.switchState(new MainMenuState());
			...
			else
			{
				...
				if (FlxG.save.data.scoreScreen)
					{
						if (SONG.song.toLowerCase() == 'your choice' || SONG.song.toLowerCase() == 'nothing personal' || SONG.song.toLowerCase() == 'training') //add news songs names to get freeplay to get back on the menu after the end of the song -Z
							{
								FlxG.switchState(new FreeplayState());
							}
					else
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curSong == 'Training' && dad.curCharacter == 'gf') {
		...
		if (FlxG.save.data.camzoom)
		{
			...
			if (curSong.toLowerCase() == 'nothing personal' && curStep >= 192 && curStep < 256 && camZooming && FlxG.camera.zoom < 1.35)
				{
					FlxG.camera.zoom += 0.15;
					camHUD.zoom += 0.03;
				}

			if (curSong.toLowerCase() == 'your choice' && curStep >= 320 && curStep < 400 && camZooming && FlxG.camera.zoom < 1.35)
				{
					FlxG.camera.zoom += 0.15;
					camHUD.zoom += 0.03;
				}
			if (curSong.toLowerCase() == 'your choice' && curStep >= 832 && curStep < 960 && camZooming && FlxG.camera.zoom < 1.35)
				{
					FlxG.camera.zoom += 0.15;
					camHUD.zoom += 0.03;
				}
		...
		if (dad.curCharacter == 'gf' && SONG.song.toLowerCase() == 'training')
			{
		...
		if (curSong == 'Nothing Personal' && dad.curCharacter == 'jix') {
			if (curBeat % 2 == 0 && dad.animOffsets.exists('idle'))
				dad.playAnim('idle');
		}
		
		if (curSong == 'Your Choice' && dad.curCharacter == 'jix') {
			if (curBeat % 2 == 0 && dad.animOffsets.exists('idle'))
				dad.playAnim('idle');
		}
